...
```
paraestra/
├── index.html      ← 홈 (인트로 + ENTER + 영상 → 히어로 + 메뉴 카드) + SEO 풀셋
├── about.html      ← 관장 소개(전용재 블랙벨트 3단) + 계보(나카이 유키)
├── program.html    ← 프로그램 (주짓수 / MMA)
├── schedule.html   ← 시간표 + 체육관 특징
├── gallery.html    ← 갤러리 (추후 사진 페이지로 확장)
├── branches.html   ← 전국 지점 안내 (지역별 카드 그리드)
...
홈 페이지에 아래 항목이 설정되어 있습니다.

- `<title>` / `<meta description>` / `<meta keywords>`
- Open Graph (카카오톡·페이스북 공유 미리보기)
- Twitter Card
- JSON-LD Schema Markup — `SportsActivityLocation` 타입 (Google 지식 패널 대응)

**도메인 확정 후** index.html 에서 `USERNAME` 을 실제 주소로 교체하세요 (4군데, `<!-- TODO -->` 주석 표시).

## 전국 지점 안내 (branches.html)

지역별 섹션(청주·충북·수도권·충남·대구·울산·전남·제주)으로 구성된 카드 그리드.
지점 추가/삭제는 해당 region-block 안의 `branch-card` 블록을 복붙하거나 삭제하면 됩니다.

```html
<div class="branch-card">
  <div class="bc-name">지점명</div>
  <div class="bc-addr">주소</div>
  <div class="bc-tel"><a href="tel:000-0000-0000">000-0000-0000</a></div>
</div>
```
